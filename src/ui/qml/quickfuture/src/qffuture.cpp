...
static int typeId(const QVariant &v) { return v.userType(); }
...
void Future::registerType(int typeId, VariantWrapperBase *wrapper) {
    if (m_wrappers.contains(typeId)) {
        qWarning() << QString("QuickFuture::registerType:It is already registered:%1")
...
QJSEngine *Future::engine() const { return m_engine; }
...
bool Future::isFinished(const QVariant &future) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return false;
    }
...
bool Future::isCanceled(const QVariant &future) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future.isCanceled: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return false;
    }
...
int Future::progressValue(const QVariant &future) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future.progressValue: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return false;
    }
...
int Future::progressMinimum(const QVariant &future) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future.progressMinimum: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return false;
    }
...
int Future::progressMaximum(const QVariant &future) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future.progressMaximum: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return false;
    }
...
void Future::onCanceled(const QVariant &future, QJSValue func, QJSValue owner) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(static_cast<int>(future.type())));
        return;
    }
    VariantWrapperBase *wrapper = m_wrappers[typeId(future)];
...
void Future::onProgressValueChanged(const QVariant &future, QJSValue func) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString(
                          "Future.onProgressValueChanged: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return;
    }
    VariantWrapperBase *wrapper = m_wrappers[typeId(future)];
...
void Future::sync(
    const QVariant &future,
    const QString &propertyInFuture,
    QObject *target,
    const QString &propertyInTarget) {
    if (!m_wrappers.contains(typeId(future))) {
        qWarning() << QString("Future: Can not handle input data type: %1")
                          .arg(QMetaType::typeName(future.type()));
        return;
