...
void SessionUI::set_backend(caf::actor backend) {
    ...
    try {
        auto actor = request_receive<caf::actor>(*sys, backend_, tag::get_tag_atom_v);
...
void SessionUI::updateDataSources() {
...
void SessionUI::update_preferences(const utility::JsonStore &js) {
    static auto media_flags_cache = JsonStore();

    try {
        auto flags = global_store::preference_value<JsonStore>(js, "/core/session/media_flags");
        if (media_flags_cache != flags) {
            media_flags_cache = flags;
            media_flags_.clear();
            for (const auto &i : flags) {
                media_flags_.append(new FlagObject(i["name"], i["colour"], this));
            }
            emit mediaFlagsChanged();
        }
...
void SessionUI::init(actor_system &system_) {
    ...
    // disable testing of shotgun plugin.
    updateDataSources();
    ...
    set_message_handler([=](actor_companion * /*self_*/) -> message_handler {
        return {
            ...
            [=](utility::event_atom, playlist::reflag_container_atom, const Uuid & /*uuid*/) {
                updateItemModel();
                emit itemModelChanged();
            },
            ...
            [=](utility::event_atom, playlist::rename_container_atom, const Uuid & /*uuid*/) {
                updateItemModel();
                emit itemModelChanged();
            },
...
QUuid SessionUI::addTag(
    ...
    try {
        ...
        scoped_actor sys{system()};
        auto tag_actor = request_receive<caf::actor>(*sys, backend_, tag::get_tag_atom_v);
