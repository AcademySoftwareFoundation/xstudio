...
MacOSAudioOutputDevice::MacOSAudioOutputDevice(const utility::JsonStore &prefs)
    ...
    try {
        ...
        buffer_size_ = preference_value<long>(prefs_, "/core/audio/pulse_audio_prefs/buffer_size");
        sample_rate_ = preference_value<long>(prefs_, "/core/audio/pulse_audio_prefs/sample_rate");
        ...
        num_channels_ = preference_value<long>(prefs_, "/core/audio/pulse_audio_prefs/channels");
...
void output_buffer_cb(void * inUserData,
          AudioQueueRef           inAQ,
...
void MacOSAudioOutputDevice::disconnect_from_soundcard() { 
    ...
    try {
        ...
        audio_out_data_.reset();
        throw std::runtime_error(fmt::format("Failed to create output audio queue with OSStatus error code {}",b).c_str());
    }
...
void MacOSAudioOutputDevice::connect_to_soundcard() { 
    ...
    try {
        ...
        AudioQueueSetParameter(audio_out_data_->audio_buffer_queue_, kAudioQueueParam_Pitch, pitch);*/
        audio_out_data_->start();
...
long MacOSAudioOutputDevice::desired_samples() { 
    ...
    if (latency_microseconds() > aprx_buffer_water_level_microsecs_) return 0;
...
long MacOSAudioOutputDevice::latency_microseconds() { 
    ...
    if (audio_out_data_->last_sample_position_in_queue_ > ts.mSampleTime) {
        // the last time we enqueued a buffer, we got information about the sample time for the first
        // sample in that buffer - we subtract the current sample time from that an apply the sample
        // rate. We also add the duration of the last buffer ... this tells us the total duration
        // of the audio queued for playback
        auto v = ((audio_out_data_->last_sample_position_in_queue_-ts.mSampleTime)*1000000)/sample_rate_;
        return ((audio_out_data_->last_sample_position_in_queue_-ts.mSampleTime)*1000000)/sample_rate_;
    }
...
bool MacOSAudioOutputDevice::push_samples(const void *sample_data, const long num_samples) {
    ...
    if (!audio_out_data_) return false;
    ...
    audio_out_data_->last_sample_position_in_queue_ = actual_play_time.mSampleTime + num_samples/2;
    ...
    //std::this_thread::sleep_for(std::chrono::milliseconds(audio_out_data_->last_buffer_duration_microsecs_ / 1200));
    return true;
}
