...
TEST(TimelineActorChildTest, Test) {
    ...
    // auto result = request_receive<bool>(*(f.self), t, insert_item_atom_v, 0, UuidActor(uuid,
    // stack1));
    ...
    auto result = request_receive<JsonStore>(
        *(f.self), stack1, insert_item_atom_v, 0, UuidActor(uuid, track1));
    ...
    result     = request_receive<JsonStore>(
        *(f.self),
        ...
        FrameRange(
            ...
            FrameRateDuration(3, timebase::k_flicks_24fps)));
    result = request_receive<JsonStore>(
        *(f.self), track1, insert_item_atom_v, -1, UuidActor(uuid, clip1));

    // stack
    uuid.generate_in_place();
    auto stack2 = f.self->spawn<StackActor>("Nested Stack-002", uuid);
    result      = request_receive<JsonStore>(
        *(f.self), track1, insert_item_atom_v, -1, UuidActor(uuid, stack2));
    result = request_receive<JsonStore>(
        *(f.self),
        stack2,
        active_range_atom_v,
    ...
    result = request_receive<JsonStore>(
        *(f.self), track1, insert_item_atom_v, -1, UuidActor(uuid, gap1));
    ...
    result     = request_receive<JsonStore>(
        *(f.self),
        ...
        FrameRange(
            ...
            FrameRateDuration(6, timebase::k_flicks_24fps)));
    result = request_receive<JsonStore>(
        *(f.self), track1, insert_item_atom_v, -1, UuidActor(uuid, clip4));
    ...
    result = request_receive<JsonStore>(
        *(f.self), stack2, insert_item_atom_v, -1, UuidActor(uuid, track2));

    uuid.generate_in_place();
    auto track3 =
        f.self->spawn<TrackActor>("Nested Track-003", media::MediaType::MT_IMAGE, uuid);
    result = request_receive<JsonStore>(
        *(f.self), stack2, insert_item_atom_v, -1, UuidActor(uuid, track3));
    result = request_receive<JsonStore>(
        *(f.self),
        track3,
        active_range_atom_v,
    ...
    result = request_receive<JsonStore>(
        *(f.self), track2, insert_item_atom_v, -1, UuidActor(uuid, gap2));
    ...
    result = request_receive<JsonStore>(
        *(f.self), track2, insert_item_atom_v, -1, UuidActor(uuid, clip3));
    ...
    result     = request_receive<JsonStore>(
        *(f.self),
        ...
        FrameRange(
            ...
            FrameRateDuration(9, timebase::k_flicks_24fps)));
    result = request_receive<JsonStore>(
        *(f.self), track3, insert_item_atom_v, -1, UuidActor(uuid, clip5));
    ...
    result     = request_receive<JsonStore>(
        *(f.self),
        ...
        FrameRange(
            ...
            FrameRateDuration(3, timebase::k_flicks_24fps)));
    result = request_receive<JsonStore>(
        *(f.self), track3, insert_item_atom_v, -1, UuidActor(uuid, clip6));
...
TEST(TimelineActorHistoryTest, Test) {
    ...
    result          = request_receive<JsonStore>(
        *(f.self), track3, insert_item_atom_v, -1, UuidActor(clip6_uuid, clip6));
    result = request_receive<JsonStore>(
        *(f.self),
        clip6,
        active_range_atom_v,
