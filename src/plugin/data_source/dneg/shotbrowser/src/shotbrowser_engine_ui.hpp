...
class ShotBrowserEngine : public QMLActor {
    ...
    Q_PROPERTY(QString shotGridUserName READ shotGridUserName NOTIFY shotGridUserNameChanged)
    Q_PROPERTY(
        bool shotGridLoginAllowed READ shotGridLoginAllowed NOTIFY shotGridLoginAllowedChanged)
    ...
    Q_PROPERTY(
        QVariant liveLinkKey READ liveLinkKey WRITE setLiveLinkKey NOTIFY liveLinkKeyChanged)
  ...
  public slots:
    ...
    QUrl
    getSequencePath(const QStringList &preferred, const QString &project, const QUuid &stalk) {
    ...
    QString getPlaylistVersions(const int id) { return getPlaylistVersionsFuture(id).result(); }
    QFuture<QString> getPlaylistVersionsFuture(const int id);

    QString getPlaylistNotes(const int id) { return getPlaylistNotesFuture(id).result(); }
    ...
    QFuture<QString> addVersionsToPlaylistFuture(
        const QVariantMap &versions, const QUuid &playlist, const QUuid &before = QUuid());
    ...
    QString getShots(const int project_id) { return getShotsFuture(project_id).result(); }
    ...
    QString
    tagEntityFromName(const QString &entity, const int record_id, const QString &tagName) {
        return tagEntityFromNameFuture(entity, record_id, tagName).result();
    }
    QFuture<QString>
    tagEntityFromNameFuture(const QString &entity, const int record_id, const QString &tagName);
    ...
    QString refreshPlaylistVersions(const QUuid &playlist, const bool matchOrder = false) {
    ...
    QFuture<QString> pushPlaylistNotesFuture(const QString &notes, const QUuid &playlist);
    ...
    QString requestFileTransfer(
        ...
        return requestFileTransferFuture(items, project, src_location, dest_location, deps)
            .result();
    }
    ...
    QVariant mergeQueries(
        const QVariant &dst, const QVariant &src, const bool ignore_duplicates = true) const;

    // value used by query, may map to id.

    void updateQueryValueCache(
        const std::string &type, const utility::JsonStore &data, const int project_id = -1);

    QFuture<QString> addDownloadToMediaFuture(const QUuid &uuid);
    QFuture<QVariant> refreshMetadataFuture(const QUuid &uuid);

    QVariant refreshMetadata(const QUuid &uuid) { return refreshMetadataFuture(uuid).result(); }
    ...
    QUrl downloadMedia(
        ...
        const QString &parent_name) {
        try {
            return downloadMediaFuture(entity_id, entity_name, project_name, parent_name)
                .result();
    ...
    QUrl downloadImage(
        ...
        const QString &project_name) {
        try {
            return downloadImageFuture(entity_id, entity, entity_name, project_name).result();
        } catch (const std::exception &err) {
  ...
  private:
    ...
    void
    receivedDataSlot(const QPersistentModelIndex &index, const int role, const QString &result);
