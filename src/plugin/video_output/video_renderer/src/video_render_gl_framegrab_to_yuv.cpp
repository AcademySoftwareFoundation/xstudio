...
const char *frag_shader = R"(
    ...
    void main(void)
    {
        if (image_st.y < 2.0/3.0) {
            vec2 tx_coord = vec2(image_st.x, 1.0-image_st.y*3.0/2.0);
            FragColor.r = dot(vec3(0.257, 0.504, 0.098), texture(vp_tex, tx_coord).rgb);
        } else if (image_st.y < 5.0/6.0) {
            vec2 tx_coord = vec2(image_st.x < 0.5? image_st.x*2.0 : (image_st.x-0.5)*2.0, 1.0-(image_st.y*6.0-4.0));
            FragColor.r = dot(vec3(-0.148, -0.291, 0.439), texture(vp_tex, tx_coord).rgb)+ 0.5;
        } else {
            vec2 tx_coord = vec2(image_st.x < 0.5? image_st.x*2.0 : (image_st.x-0.5)*2.0, 1.0-(image_st.y*6.0-5.0));
            FragColor.r = dot(vec3(0.439, -0.368, -0.071), texture(vp_tex, tx_coord).rgb) + 0.5;
        }
    }
...
YUVFrameGrabber::~YUVFrameGrabber() {
    if (vbo_) {
...
void YUVFrameGrabber::viewport_capture_gl_framebuffer(
    ...
    setupYUVImageTextureAndFrameBuffer(fb_width, fb_height);
    ...
    grabYUVFrameBuffer(fb_width, fb_height * 3 / 2, destination_image);
}

void YUVFrameGrabber::setupYUVImageTextureAndFrameBuffer(const int width, const int __height) {

    // YUV420 planar - U, V plaes are 1/4 the size of the image. In rectilinear
    // pixel coords, the U channel is stacked ontop of the Y plane and the V
    // plane packed on-top of the U.
    const int height = __height * 3 / 2;
...
void YUVFrameGrabber::grabYUVFrameBuffer(
    ...
    size_t pix_buf_size = width * height * sizeof(uint8_t);
    ...
    glGetTexImage(GL_TEXTURE_2D, 0, GL_RED, GL_UNSIGNED_BYTE, nullptr);
