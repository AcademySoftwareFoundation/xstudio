...
inline void unpack_json_array(utility::JsonStore &json) {
    if (not json.empty()) {
        throw std::runtime_error("unpack_json_array, array has more elements than expected");
    }
}
...
void unpack_json_array(utility::JsonStore &json, T &f, Args &...args) {
    if (!json.is_array()) {
        ...
        throw std::runtime_error("unpack_json_array, array has fewer elements than expected");
    }
    f = json[0].get<T>();
