...
namespace bm_decklink_plugin_1_0 {
      ...
      public:
        SimpleThreadPool(
            const int num_threads =
                std::max(8, static_cast<int>(std::thread::hardware_concurrency() / 2))) {
        ...
        template <class JobType, typename... Args> void add_job(Args &&...args) {
            {
                std::lock_guard lk(m);
    ...
    struct RGBA16_to_10bitRGB : public JobFunc {
        RGBA16_to_10bitRGB(uint16_t *src, uint32_t *dst, size_t n)
            : _src(src), _dst(dst), n(n) {}
        uint16_t *_src;
        uint32_t *_dst;
        size_t n;
        void doit() override;
    ...
    struct RGBA16_to_10bitRGBX : public JobFunc {
        RGBA16_to_10bitRGBX(uint16_t *src, uint32_t *dst, size_t n)
            : _src(src), _dst(dst), n(n) {}
        uint16_t *_src;
        uint32_t *_dst;
        size_t n;
    ...
    struct RGBA16_to_10bitRGBXLE : public JobFunc {
        RGBA16_to_10bitRGBXLE(uint16_t *src, uint32_t *dst, size_t n)
            : _src(src), _dst(dst), n(n) {}
        uint16_t *_src;
        uint32_t *_dst;
        size_t n;
    ...
    struct RGBA16_to_12bitRGBLE : public JobFunc {
        RGBA16_to_12bitRGBLE(uint16_t *src, uint16_t *dst, size_t n)
            : _src(src), _dst(dst), n(n) {}
        uint16_t *_src;
        uint16_t *_dst;
        size_t n;
    ...
    struct RGBA16_to_12bitRGB : public JobFunc {
        RGBA16_to_12bitRGB(uint16_t *src, uint16_t *dst, size_t n)
            : _src(src), _dst(dst), n(n) {}
        uint16_t *_src;
        uint16_t *_dst;
        size_t n;
