...
static const std::vector<std::string> hdr_metadata_value_names(
    {"Red x", "Red y", "Green x", "Green y", "Blue x", "Blue y", "White Pt. x", "White Pt. y"});

const std::vector<std::string>
    light_level_controls_names({"D.M. Lum. Min", "D.M. Lum. Max", "CLL Max", "Frm Avg Light"});
...
BMDecklinkPlugin::BMDecklinkPlugin(
    ...
    pixel_formats_ = add_string_choice_attribute(
        "Pixel Format", "Pix Fmt", "10 bit YUV", utility::map_key_to_vec(bmd_pixel_formats));
    ...
    resolutions_ = add_string_choice_attribute("Output Resolution", "Out. Res.");
    ...
    frame_rates_ = add_string_choice_attribute("Refresh Rate", "Hz");
    ...
    start_stop_ = add_boolean_attribute("Start Stop", "Start Stop", false);
    ...
    samples_water_level_ =
        add_integer_attribute("Audio Samples Water Level", "Audio Samples Water Level", 4096);
    samples_water_level_->set_preference_path("/plugin/decklink/audio_samps_water_level");

    // This attr is currently not exposed to the user. It can be used to delay (or advance) the
    // image that is sent to the SDI output vs. the main xSTUDIO viewport in order to exactly
    // sync up the image that shows in xSTUDIO UI vs. the SDI display. This helps if you have a
    // screening room with xSTUDIO UI on a monitor and the SDI output shown by a projector at
    // the front of the room ... the person operating xSTUDIO may see the image on their screen
    // is a few frames ahead of the SDI. However, for this to work we have to buffer a lot more
    // frames in the playhead and this is expensive which is why I'm leaving it here but not
    // exposing it.
    video_pipeline_delay_milliseconds_ =
        add_integer_attribute("Video Sync Delay", "Video Sync Delay", 0);
    video_pipeline_delay_milliseconds_->set_preference_path(
        "/plugin/decklink/video_sync_delay");
    video_pipeline_delay_milliseconds_->expose_in_ui_attrs_group("Decklink Settings");

    audio_sync_delay_milliseconds_ =
        add_integer_attribute("Audio Sync Delay", "Audio Sync Delay", 0);
    audio_sync_delay_milliseconds_->set_preference_path(
        "/plugin/decklink/new_audio_sync_delay");
    audio_sync_delay_milliseconds_->expose_in_ui_attrs_group("Decklink Settings");

    disable_pc_audio_when_running_ =
        add_boolean_attribute("Auto Disable PC Audio", "Auto Disable PC Audio", false);
    disable_pc_audio_when_running_->set_preference_path(
        "/plugin/decklink/disable_pc_audio_when_sdi_is_running");
...
void BMDecklinkPlugin::receive_status_callback(const utility::JsonStore &status_data) {
    ...
    if (status_data.contains("sdi_output_is_active") &&
        status_data["sdi_output_is_active"].is_boolean()) {
    ...
    if (dcl_output_) {
        ...
        if (attribute_uuid == pixel_formats_->uuid() ||
            ...
            try {
                ...
                if (bmd_pixel_formats.find(pixel_formats_->value()) ==
                    bmd_pixel_formats.end()) {
                    throw std::runtime_error(
                        fmt::format("Invalid pixel format: {}", pixel_formats_->value()));
                }
                ...
                dcl_output_->set_display_mode(
                    resolutions_->value(), frame_rates_->value(), pix_fmt);
...
void BMDecklinkPlugin::initialise() {
    ...
    try {
        ...
        // transform
        display_info("SDI Video Output", "Decklink", "Blackmagic Design", "");
        ...
        anon_mail(
            ...
            utility::JsonStore(
                nlohmann::json::parse(R"(["decklink viewport attrs", "decklink ocio"])")))
...
void BMDecklinkPlugin::set_pc_audio_muting() {
    ...
    if (pc_audio_output_actor) {
        const bool mute =
            disable_pc_audio_when_running_->value() && sdi_output_is_running_->value();
        anon_send(
            pc_audio_output_actor, audio::set_override_volume_atom_v, mute ? 0.0f : -1.0f);
    }
}
...
void BMDecklinkPlugin::make_hdr_attributes() {
    ...
    hdr_mode_ = add_string_choice_attribute(
    ...
    try {
        ocio_display_hdr_match_string_ = utility::to_lower(
            prefs.value<std::string>("/plugin/decklink/ocio_display_hdr_match_string"));
    } catch (std::exception &e) {
    ...
    try {
        ...
        if (colour_vals.is_array() && colour_vals.size() == 8) {
            ...
            for (auto &o : colour_vals) {
                if (!o.is_number()) {
                    throw std::runtime_error(
                        "hdr_colour_metadata_values should be 8 float values.");
                }
                colour_settings[idx++] = o.get<float>();
            }
        } else {
            throw std::runtime_error(
                "hdr_colour_metadata_values should be an array of 8 float values.");
        }
    } catch (std::exception &e) {
        spdlog::warn(
            "{}: Failed to load perference /plugin/decklink/hdr_colour_metadata_values: {}",
            __PRETTY_FUNCTION__,
            e.what());
    }
    ...
    try {
        ...
        if (lum__vals.is_array() && lum__vals.size() == 4) {
            ...
            for (auto &o : lum__vals) {
                if (!o.is_number()) {
                    throw std::runtime_error(
                        "hdr_luminance_metadata_values should be 4 float values.");
                }
                lum_settings[idx++] = o.get<float>();
            }
        } else {
            throw std::runtime_error(
                "hdr_luminance_metadata_values should be an array of 4 float values.");
        }
    } catch (std::exception &e) {
        spdlog::warn(
            "{}: Failed to load perference /plugin/decklink/hdr_luminance_metadata_values: {}",
            __PRETTY_FUNCTION__,
            e.what());
    }
    ...
    try {
        ...
        hdr_presets_data_ = prefs.value<utility::JsonStore>("/plugin/decklink/hdr_presets");
        ...
        spdlog::warn(
            "{}: Failed to load perference /plugin/decklink/hdr_presets: {}",
            __PRETTY_FUNCTION__,
            e.what());
    }
}
...
void BMDecklinkPlugin::set_hdr_mode_and_metadata() {
    ...
    if (metadata.EOTF == -1) {
        // auto turn on HDR mode if the ocio display includes the string
        // 'ocio_display_hdr_match_string_' which was set from our preferences. So for example,
        // if ocio_display_hdr_match_string_ is 'hdr' then if the ocio display is 'Screening
        // Room (HDR)' then HDR output is turned on
        if (utility::to_lower(get_ocio_display_name()).find(ocio_display_hdr_match_string_) !=
            std::string::npos) {
...
std::string BMDecklinkPlugin::get_ocio_display_name() {
    ...
    try {
        ...
        auto ocio_display = utility::request_receive<utility::JsonStore>(
            *sys, colour_pipeline(), module::attribute_value_atom_v, std::string("Display"));
