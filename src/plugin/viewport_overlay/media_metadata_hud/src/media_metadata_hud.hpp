...
class MediaMetadata : public utility::BlindDataObject {
  public:
    MediaMetadata(const DisplaySettingsPtr &d, const bool grid_layout)
        : display_settings_(d), last_used_(utility::clock::now()), grid_layout_(grid_layout) {}
...
class MediaMetadataHUD : public plugin::HUDPluginBase {
  ...
  private:
    ...
    void set_profile_field_label(const std::string &field, const std::string &field_label);
    void
    set_field_screen_position(const std::string &field, const std::string &screen_position);
