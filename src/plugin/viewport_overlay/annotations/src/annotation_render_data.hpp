...
class LaserStrokesRenderDataSet : public utility::BlindDataObject {
  public:
    ...
    void
    add_laser_strokes(const std::vector<std::shared_ptr<ui::canvas::Stroke>> &laser_strokes) {
...
class PerImageAnnotationRenderDataSet : public utility::BlindDataObject {
  public:
    ...
    void add_erase_stroke(const canvas::Stroke *stroke, const utility::Uuid &bookmark_uuid) {
        live_erase_strokes_[bookmark_uuid].push_back(std::make_shared<canvas::Stroke>(*stroke));
    }

    void add_live_caption(const canvas::Caption *caption, const HandleHoverState hstate) {
        captions_.push_back(std::make_shared<canvas::Caption>(*caption));
        handles_.push_back(hstate);
    }

    void add_hovered_caption_box(const Imath::Box2f &box) { hovered_boxes_.push_back(box); }
    ...
    const utility::Uuid &skip_annotation_uuid() const { return skip_render_annotation_uuid_; }
    ...
    const std::vector<Imath::Box2f> &hovered_caption_boxes() const { return hovered_boxes_; }

    const StrokeVec &live_erase_strokes(const utility::Uuid &affected_bookmark_id) const {
