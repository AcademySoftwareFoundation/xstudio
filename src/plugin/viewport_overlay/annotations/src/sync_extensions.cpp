...
std::string timestmp() {
    ...
    std::strftime(
        timeString.data(), timeString.size(), "%Y-%m-%dT%H:%M:%S.%fZ", std::gmtime(&time));
    return std::string(timeString.data());
}

static const nlohmann::json paintStartTemplate = nlohmann::json::parse(
    R"(
{
...
static const nlohmann::json paintPointTemplate = nlohmann::json::parse(
    R"(
{
    "schema": "SYNC_REVIEW_1.0",
    "payload": {
...
static const nlohmann::json paintEndTemplate = nlohmann::json::parse(
    R"(
{
    "schema": "SYNC_REVIEW_1.0",
    "payload": {
...
static const nlohmann::json paintClearTemplate = nlohmann::json::parse(
    R"(
{
    "schema": "SYNC_REVIEW_1.0",
    "payload": {
...
void AnnotationsTool::paint_start_event(const Imath::V2f &point) {
    ...
    paintStartData["payload"]["command"]["payload"]["paint"]["layer_range"]["start_time"]
                  ["value"] = int(image_being_annotated_.playhead_logical_frame());
    paintStartData["payload"]["command"]["payload"]["paint"]["layer_range"]["start_time"]
                  ["rate"] =
                      1.0 / timebase::to_seconds(image_being_annotated_.frame_id().rate());

    paintStartData["payload"]["command"]["payload"]["paint"]["layer_range"]["duration"]
                  ["value"] = 1;
    paintStartData["payload"]["command"]["payload"]["paint"]["layer_range"]["duration"]
                  ["rate"] =
                      1.0 / timebase::to_seconds(image_being_annotated_.frame_id().rate());

    mail(utility::event_atom_v, ui::viewport::annotation_atom_v, paintStartData)
        .send(plugin_events_group());
}

void AnnotationsTool::paint_point_event(const Imath::V2f &point) {

    if (paint_stroke_id_.is_null())
        return;
    ...
    paintPointData["payload"]["command"]["payload"]["point_target"]["range"]["start_time"]
                  ["value"] = int(image_being_annotated_.playhead_logical_frame());
    paintPointData["payload"]["command"]["payload"]["point_target"]["range"]["start_time"]
                  ["rate"] =
                      1.0 / timebase::to_seconds(image_being_annotated_.frame_id().rate());

    paintPointData["payload"]["command"]["payload"]["point_target"]["range"]["duration"]
                  ["rate"] =
                      1.0 / timebase::to_seconds(image_being_annotated_.frame_id().rate());

    paintPointData["payload"]["command"]["payload"]["point"]["x"] = point.x;
    paintPointData["payload"]["command"]["payload"]["point"]["y"] = -point.y;
    paintPointData["payload"]["command"]["payload"]["point"]["size"] =
        draw_pen_size_->value() / PEN_STROKE_THICKNESS_SCALE;

    mail(utility::event_atom_v, ui::viewport::annotation_atom_v, paintPointData)
        .send(plugin_events_group());
}
...
void AnnotationsTool::paint_end_event() {
    ...
    mail(utility::event_atom_v, ui::viewport::annotation_atom_v, paintEndData)
        .send(plugin_events_group());
...
void AnnotationsTool::incoming_paint_event(const utility::JsonStore &command_data) {
    ...
    try {
        ...
        if (event == "PaintStart") {
            ...
            if (command_data.contains(RGBA_JPOINTER)) {
                ...
                if (rgba.is_array() && rgba.size() == 4) {
                    ...
                    utility::ColourTriplet rgb(
                        rgba[0].get<float>(), rgba[1].get<float>(), rgba[2].get<float>());
        ...
        spdlog::warn(
            "{} Failed to porcess event payload relating to: {}",
            __PRETTY_FUNCTION__,
            e.what());
    }
    do_redraw();
