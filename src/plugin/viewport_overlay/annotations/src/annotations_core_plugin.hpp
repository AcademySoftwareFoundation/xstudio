        ...
        class AnnotationsCore : public plugin::StandardPlugin {
          ...
          protected:
            ...
            void make_bookmark_for_annotations(
              const media::AVFrameID &id,
              const utility::Uuid & mb_id
            );
          ...
          private:
            ...
            void pick_image_to_annotate(
              const Imath::V2f &pos,
              LiveEditData &live_edit_Data);
            ...
            void schedule_bookmark_update(
              LiveEditData & user_edit_data);

            Annotation *modifiable_annotation(
              LiveEditData &user_edit_data);

            void remove_live_caption(
              LiveEditData &user_edit_data);

            void clear_live_caption(
              LiveEditData &user_edit_data);

            void caption_key_press(
              const utility::JsonStore &payload,
              LiveEditData & user_edit_data);
            ...
            LiveEditData & live_edit_data(const utility::Uuid & uuid) {
              ...
              if (p != live_edit_data_.end()) return p->second;
            ...
            const media_reader::ImageBufDisplaySetPtr &get_viewport_image_set(const std::string &viewport_name) {
