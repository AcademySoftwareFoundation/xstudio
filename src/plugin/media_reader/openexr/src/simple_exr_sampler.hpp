...
class SimpleExrSampler {
  public:
    SimpleExrSampler(ImageBufPtr exr_buf, thumbnail::ThumbnailBufferPtr &buf)
        ...
        exr_bytes_per_line = (exr_data_win.max.x - exr_data_win.min.x) * exr_bytes_per_pixel;
    ...
    struct RGB {
        ...
        RGB() : r(0), g(0), b(0) {}
        RGB(const float l) {
            r = uint8_t(round(std::max(std::min(pow(l, 1.0f / 2.2f) * 255.0f, 255.0f), 0.0f)));
            g = r;
            b = r;
        }

        RGB(const float _r, const float _g, const float _b) {
            r = uint8_t(round(std::max(std::min(pow(_r, 1.0f / 2.2f) * 255.0f, 255.0f), 0.0f)));
            g = uint8_t(round(std::max(std::min(pow(_g, 1.0f / 2.2f) * 255.0f, 255.0f), 0.0f)));
            b = uint8_t(round(std::max(std::min(pow(_b, 1.0f / 2.2f) * 255.0f, 255.0f), 0.0f)));
        }
