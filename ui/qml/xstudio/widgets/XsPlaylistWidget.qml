...
Rectangle {
    ...
    property bool is_drag_drop_target: false
 	...
 	XsSessionBarWidget {
        ...
        decoratorModel: (
            session.tags[backend.uuid] ?
                (
                    session.tags[backend.uuid]["Decorator"] ? session.tags[backend.uuid]["Decorator"].tags : null
                )
                : null
            )
 ...
 //        clip: true
	...
	XsMenu {
        ...
        XsMenuSeparator {
            visible: session.tags[backend.uuid] ? (session.tags[backend.uuid]["Menu"] ? session.tags[backend.uuid]["Menu"].tags.length : 0 ) : 0
        }
        ...
        Repeater {
            ...
            session.tags[backend.uuid] ?
                (
                    session.tags[backend.uuid]["Menu"] ? session.tags[backend.uuid]["Menu"].tags : null
                )
                : null
            )
            XsDecoratorWidget {
