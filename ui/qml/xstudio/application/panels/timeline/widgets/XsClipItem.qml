...
XsGradientRectangle {
	...
	border.width: 1
    border.color: isHovered ? XsStyleSheet.accentColor : (isMissingMedia && isEnabled ? "Red" : (isInvalidRange && isEnabled ? "Orange" : Qt.darker(realColor, 0.8)))

	flatColor: topColor

    topColor: isSelected ? Qt.darker(XsStyleSheet.accentColor, 2) : realColor
    bottomColor: isSelected ? Qt.darker(XsStyleSheet.accentColor, 2) :  Qt.darker(realColor,1.2)
	...
	XsClipDragLeft {
		...
		shadowColor: XsStyleSheet.primaryTextColor
