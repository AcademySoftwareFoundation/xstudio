...
namespace ui {
    namespace qml {
        ...
        class TimelineUI : public QMLActor {
          ...
          public:
            ...
            [[nodiscard]] QString type() const { return "Timeline"; }
          ...
          public slots:
            ...
            void dragDropReorder(const QVariantList, const QString) {}
            void sortAlphabetically() {}

            // QList<QUuid> loadMedia(const QUrl &path) { return
            // loadMediaFuture(path).result(); } QFuture<QList<QUuid>> loadMediaFuture(const
            // QUrl &path); bool addMedia(const QUuid &uuid, const QUuid &before_uuid =
            // QUuid()); QUuid getNextItemUuid(const QUuid &quuid); bool
            // contains_media(const QUuid &key) const;
