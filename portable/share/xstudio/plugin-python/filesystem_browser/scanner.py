...
class FileScanner:
    ...
    def get_owner(self, st):
        """Get file owner from stat object. Returns username or 'unknown' on Windows."""
        if pwd:
            try:
    ...
    def _process_files(self, raw_files, start_path):
        ...
        for seq in sequences:
            ...
            owner = self.get_owner(first_info[1]) if first_info else "?"
            ...
            item = {
                ...
                "relpath": os.path.relpath(first_path, start_path).replace("\\", "/"), # Relative path of ONE file? Or sequence?
            ...
            item["relpath"] = os.path.relpath(str(seq), start_path).replace("\\", "/")
