...
Our instructions here involve building the libraries that xSTUDIO depends on and installing to your system before building xSTUDIO itself. If you do not wish to modify your system with these installations then we would recommend adding custom install paths to the build commands below. You will also need to modify the CMake seach paths when building xSTUDIO so that it can locate these packages. We assume users doing so already have experience in package management if they do so and we do not currently provide a comprehensive guide for doing this.

### Qt SDK 6.5.3

xSTUDIO now requires Qt6.5.3. You can install Qt6.5.3 using [these instructions](downloading_qt.md).
