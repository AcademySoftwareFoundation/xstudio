...
When using a pressure sensitive input device such as a graphics tablet, the freehand brush tools respond to pen pressure. The Drawing Tools box exposes a few controls to tune this behaviour: **Softness** sets the hardness of the brush edge, going from a crisp outline to a soft, feathered stroke. **Size Pressure Sensitivity** controls how strongly the stroke width reacts to pen pressure - at zero the stroke keeps a constant width, at higher values light pressure produces thin lines and heavier pressure produces thicker lines up to the configured brush size. **Opacity Pressure Sensitivity** works in the same way for stroke opacity, letting you build up colour gradually with light strokes or lay down fully opaque marks with firmer pressure.
...
The 'Dodge' and 'Burn' tools let you locally brighten or darken areas of the image by painting over them - they behave like the freehand brush but instead of laying down colour they modulate the pixels underneath. These tools are not pressure sensitive: each stroke applies a fixed amount and the effect is cumulative, so passing the brush over the same area multiple times will progressively strengthen the result. The **Intensity** slider controls how much each stroke pushes the image: low values give a subtle effect that is easy to build up gradually, high values produce a strong change in a single pass. The **Add** and **Mult** buttons select how the adjustment is applied relative to pure black: in **Add** mode the stroke value is summed with the underlying pixel, which lifts shadows and is visible even on completely black areas; in **Mult** mode the stroke value multiplies the underlying pixel, so pure black stays black and the effect scales with the brightness already present in the image.
