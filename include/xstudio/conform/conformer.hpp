...
struct ConformRequest {
    ...
    utility::JsonStore task_metadata_;
...
struct ConformReply {
    ...
    ConformReply(ConformRequest request)
        : request_(std::move(request)), operations_(ConformOperationsJSON) {}
...
class Conformer {
  public:
    ...
    virtual std::vector<std::optional<std::tuple<std::string, caf::uri, utility::JsonStore>>>
...
template <typename T> class ConformPlugin : public plugin_manager::PluginFactoryTemplate<T> {
  public:
    ConformPlugin(
        utility::Uuid uuid,
        std::string name        = "",
    ...
    ConformPluginActor(
        ...
        behavior_.assign(
            ...
            [=](conform_atom, const std::string &conform_task, const ConformRequest &request)
                -> ConformReply { return conform_.conform_request(conform_task, request); },
