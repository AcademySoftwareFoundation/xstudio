...
class PixelInfo {
  ...
  public:
    ...
    PixelInfo(const Imath::V2i &location_in_image, const std::string layer_name = std::string())
    ...
    void add_raw_channel_info(const std::string &channel_name, const float channel_value) {
        raw_data_.push_back(
            PixelChannelInfo{
                ...
                .code_value    = NullCodeValue});
    }

    void add_linear_channel_info(const std::string &channel_name, const float channel_value) {
    ...
    [[nodiscard]] const PixelChannelsData &linear_channels_info() const { return linear_data_; }
    [[nodiscard]] const PixelChannelsData &rgb_display_info() const { return screen_rgb_data_; }
