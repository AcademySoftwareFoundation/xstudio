...
class ImageBuffer : public Buffer {
  public:
    ...
    [[nodiscard]] PixelInfo pixel_info(
        const Imath::V2i &pixel_location,
        const utility::JsonStore &pixel_unpack_uniforms,
...
class ImageBufPtr : public std::shared_ptr<ImageBuffer> {
  public:
    ...
    void set_layout_transform(const Imath::M44f &t) { transient_data().layout_transform_ = t; }

    [[nodiscard]] const std::string &error_details() const {
        return transient_data().error_details_;
    }
    void set_error_details(const std::string &err) { transient_data().error_details_ = err; }

    friend float image_aspect(const ImageBufPtr &value);

    friend float image_layout_aspect(const ImageBufPtr &value);

    [[nodiscard]] utility::JsonStore metadata() const;
  ...
  private:
    ...
    std::shared_ptr<SidecarData> transient_data_{std::make_shared<SidecarData>()};

    [[nodiscard]] const SidecarData &transient_data() const { return *transient_data_.get(); }
