...
class AnnotationBase {
  public:
    ...
    AnnotationBase(const utility::JsonStore &s, const utility::Uuid &uuid)
        : bookmark_uuid_(uuid), store_(s) {}
    virtual ~AnnotationBase() = default;

    /* AnnotationBase sub-class must fill in the plugin_uuid with its respective
    plugin uuid */
    [[nodiscard]] virtual utility::JsonStore serialise(utility::Uuid &plugin_uuid) const {
        return store_;
    }
    utility::Uuid bookmark_uuid_;

    [[nodiscard]] virtual const void *user_data() const { return nullptr; }

    [[nodiscard]] virtual size_t hash() const { return 0; }
...
class Note {
  public:
    ...
    [[nodiscard]] auto subject() const { return subject_; }
    [[nodiscard]] auto note() const { return note_; }
    [[nodiscard]] auto author() const { return author_; }
    [[nodiscard]] auto created() const { return created_; }
    [[nodiscard]] auto category() const { return category_; }
...
struct BookmarkDetail {
    ...
    [[nodiscard]] utility::Timecode start_timecode_tc() const {
        if (media_reference_) {
    ...
    [[nodiscard]] std::string end_timecode() const {
        if (media_reference_) {
    ...
    [[nodiscard]] std::string duration_timecode() const {
        if (media_reference_) {
            auto fd = (*media_reference_)
                          .duration()
            ...
            // turn into timecode..
            return to_string(utility::Timecode(fd + 1, (*media_reference_).rate().to_fps()));
        }
        return "--:--:--:--";
    }

    [[nodiscard]] double get_second(const int frame) const {
        if (media_reference_) {
    ...
    [[nodiscard]] int get_frame(const double sec) const {
        if (media_reference_) {
    ...
    [[nodiscard]] int get_frame(const timebase::flicks sec) const {
        if (media_reference_) {
    ...
    [[nodiscard]] timebase::flicks duration() const {
    ...
    [[nodiscard]] std::string created() const {
    ...
    [[nodiscard]] int start_frame() const {
        if (media_reference_) {
...
class Bookmark : public utility::Container {
  public:
    ...
    [[nodiscard]] auto note() const { return note_; }
    [[nodiscard]] auto annotation() const { return annotation_; }

    [[nodiscard]] auto has_note() const { return static_cast<bool>(note_); }
    [[nodiscard]] auto has_annotation() const { return static_cast<bool>(annotation_); }
    [[nodiscard]] size_t annotation_hash() const {
        return annotation_ ? annotation_->hash() : 0;
    }

    void create_note();
    void create_annotation();

    [[nodiscard]] auto enabled() const { return enabled_; }
    [[nodiscard]] auto has_focus() const { return has_focus_; }
    [[nodiscard]] auto visible() const { return visible_; }
    [[nodiscard]] auto start() const { return start_; }
    [[nodiscard]] auto duration() const { return duration_; }
    [[nodiscard]] auto user_type() const { return user_type_; }
    [[nodiscard]] auto user_data() const { return user_data_; }
    [[nodiscard]] auto created() const { return created_; };
    ...
    void set_start(const timebase::flicks start = timebase::k_flicks_low) { start_ = start; }
