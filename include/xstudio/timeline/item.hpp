...
class Item : private Items {
  public:
    ...
    [[nodiscard]] std::optional<utility::FrameRateDuration> available_frame_duration() const;
    ...
    [[nodiscard]] utility::UuidActor uuid_actor() const { return {uuid(), actor()}; }
    ...
    bool operator==(const Item &other) const {
        return item_type_ == other.item_type_ and uuid_addr_.first == other.uuid_addr_.first and
               available_range_ == other.available_range_ and
               active_range_ == other.active_range_ and enabled_ == other.enabled_ and
               locked_ == other.locked_ and flag_ == other.flag_ and prop_ == other.prop_ and
               name_ == other.name_;
    }
  ...
  private:
    ...
    void set_range_direct(const utility::FrameRange &avail, const utility::FrameRange &active);
...
find_item(const Items &items, const utility::Uuid &uuid) {
    auto it = std::find_if(
        items.cbegin(), items.cend(), [uuid](Item const &obj) { return obj.uuid() == uuid; });
...
inline auto find_actor(Items &items, const caf::actor &actor) {
    return std::find_if(
        items.begin(), items.end(), [actor](Item const &obj) { return obj.actor() == actor; });
}
