...
template <typename T> class DataSourcePlugin : public plugin_manager::PluginFactoryTemplate<T> {
  public:
    DataSourcePlugin(
        utility::Uuid uuid,
        std::string name        = "",
...
template <typename T> class DataSourceActor : public caf::event_based_actor {
  public:
    DataSourceActor(caf::actor_config &cfg, const utility::JsonStore & = utility::JsonStore())
        ...
        behavior_.assign(
            ...
            [=](use_data_atom, const caf::actor &media, const utility::FrameRate &media_rate)
            ...
            [=](post_data_atom, const utility::JsonStore &js) -> result<utility::JsonStore> {
                try {
