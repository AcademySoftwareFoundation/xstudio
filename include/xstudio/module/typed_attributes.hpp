...
template <typename T> class TypeAttribute : public Attribute {
  ...
  public:
    ...
    void set_value(const T &v, const bool notify = true) { set_role_data(Value, v, notify); }
};
...
class BooleanAttribute : public TypeAttribute<bool> {
  public:
    BooleanAttribute(const std::string &title, const std::string &abbr_title, const bool value);
};
