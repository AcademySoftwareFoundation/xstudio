...
std::optional<ConditionalOperator> conditional_operator_from_string(const std::string &value);
...
class Op {
  public:
    ...
    virtual ~Op() = default;
    [[nodiscard]] virtual operator nlohmann::json() const { return {}; }
    [[nodiscard]] virtual utility::JsonStore serialise() const = 0;
    [[nodiscard]] std::string class_name() const { return class_name_; }
...
template <typename T> class Field : public Op {
  public:
    ...
    Field(const utility::JsonStore &jsn) : Op(jsn.at("class")) {
        ...
        if (not null_) {
            ...
            case 1: {
                ...
                if (period)
                    value_ =
                        std::pair<int32_t, Period>(jsn.at("value")[0].get<int32_t>(), *period);
            } break;
            case 2:
    ...
    [[nodiscard]] operator nlohmann::json() const override {
        return std::visit(
            [field_name = field_name_, condition = condition_, null = null_](
                ...
                if constexpr (std::is_same_v<std::monostate, TT>) {
                    result = nlohmann::json::array({field_name, to_string(condition), nullptr});
    ...
    [[nodiscard]] ConditionalOperator op() const { return condition_; }
    [[nodiscard]] std::string field() const { return field_name_; }
    ...
    [[nodiscard]] std::string json_value() const {
        return std::visit(
            [null = null_](auto &&arg) -> std::string {
                using TT = std::decay_t<decltype(arg)>;
                std::string result;
    ...
    [[nodiscard]] FieldType value() const { return value_; }
    [[nodiscard]] bool null() const { return null_; }
  ...
  public:
    ...
    [[nodiscard]] operator nlohmann::json() const override {
        return std::visit(
            [field_name = field_name_, condition = condition_, null = null_](
                ...
                using TT = std::decay_t<decltype(arg)>;
                if constexpr (std::is_same_v<std::monostate, TT>) {
                    return nlohmann::json::array({field_name, to_string(condition), nullptr});
                } else if constexpr (std::is_same_v<std::pair<int32_t, Period>, TT>) {
    ...
    [[nodiscard]] utility::JsonStore serialise() const override {
        ...
        if (value_.index() == 1) {
            ...
            std::visit(
                [jj = &jsn](auto &&arg) { (*jj)["value"] = static_cast<nlohmann::json>(arg); },
                value_);
...
template <typename T> inline std::ostream &operator<<(std::ostream &out, const Field<T> &op) {
...
class RelationType : public Field<utility::JsonStore> {
  public:
    RelationType(const std::string field_name, const std::string &class_name = "RelationType")
...
class FilterBy : public Op, private std::list<FilterType> {
  public:
    ...
    FilterBy(const BoolOperator op = BoolOperator::AND) : Op("FilterBy"), op_(std::move(op)) {}
    ~FilterBy() override = default;

    [[nodiscard]] bool enabled() const { return true; }

    [[nodiscard]] BoolOperator op() const { return op_; }
...
class Filter : public FilterBy {
  public:
    ...
    [[nodiscard]] auto entity() const { return entity_; }
    [[nodiscard]] auto project() const { return project_; }
    [[nodiscard]] auto project_id() const { return project_id_; }
...
class ShotgunClient {
  public:
    ...
    [[nodiscard]] bool failed_authentication() const { return failed_authentication_; }
    ...
    [[nodiscard]] httplib::Headers get_auth_headers(const std::string &content_type = "") const;
...
class AuthenticateShotgun {
  public:
    ...
    AuthenticateShotgun(const utility::JsonStore &jsn) {
        if (jsn.count("authentication_method") && not jsn["authentication_method"].is_null())
    ...
    [[nodiscard]] auto authentication_method() const { return authentication_method_; }
    [[nodiscard]] auto client_id() const { return client_id_; }
    [[nodiscard]] auto client_secret() const { return client_secret_; }
    [[nodiscard]] auto username() const { return username_; }
    [[nodiscard]] auto password() const { return password_; }
    [[nodiscard]] auto session_token() const { return session_token_; }
    [[nodiscard]] auto session_uuid() const { return session_uuid_; }
    [[nodiscard]] auto refresh_token() const { return refresh_token_; }
