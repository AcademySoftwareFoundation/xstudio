...
class Playlist : public utility::Container {
  public:
    ...
    [[nodiscard]] bool empty() const { return media_list_.empty() && container_tree_.empty(); }

    [[nodiscard]] const utility::PlaylistTree &containers() const { return container_tree_; }
    ...
    bool
    move_media(const utility::Uuid &uuid, const utility::Uuid &uuid_before = utility::Uuid()) {
        return media_list_.move(uuid, uuid_before);
    }
    utility::Uuid next_media(const utility::Uuid &uuid) { return media_list_.next_uuid(uuid); }
