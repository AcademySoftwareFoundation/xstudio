...
class MediaHook : public module::Module {
  ...
  public:
    ...
    gather_media_sources(
        ...
        return std::vector<std::tuple<std::string, caf::uri, xstudio::utility::FrameList>>();
    }
  ...
  protected:
    ...
    [[nodiscard]] std::string default_path() const { return "/metadata/external/" + name_; }

  private:
    const std::string name_;
};

template <typename T> class MediaHookPlugin : public plugin_manager::PluginFactoryTemplate<T> {
  public:
    MediaHookPlugin(
        utility::Uuid uuid,
        std::string name        = "",
...
template <typename T> class MediaHookActor : public caf::event_based_actor {
  ...
  public:
    MediaHookActor(caf::actor_config &cfg, const utility::JsonStore & = utility::JsonStore())
        ...
        behavior_.assign(
            ...
            [=](media_hook::gather_media_sources_atom,
                ...
                -> std::vector<std::tuple<std::string, caf::uri, xstudio::utility::FrameList>> {
            ...
            [=](media_hook::detect_display_atom,
                ...
                return media_hook_.detect_display(name, model, manufacturer, serialNumber, jsn);
            },
