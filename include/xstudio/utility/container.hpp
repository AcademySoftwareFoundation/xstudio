...
class Container {
  public:
    ...
    [[nodiscard]] caf::actor event_group() const { return event_group_; }

    void send_changed(const time_point last_changed = utility::clock::now()) {
        if (last_changed > last_changed_) {
            last_changed_ = std::move(last_changed);
            actor_->mail(event_atom_v, last_changed_atom_v, last_changed_).send(event_group_);
        }
    }
    ...
    auto make_last_changed_setter(caf::actor grp, caf::blocking_actor *act) {
        return [=](last_changed_atom, const time_point &last_changed) {
            if (last_changed > last_changed_ or last_changed == time_point()) {
                last_changed_ = last_changed;
                act->mail(utility::event_atom_v, last_changed_atom_v, last_changed_).send(grp);
            }
        };
    }
    ...
    // }
    auto make_get_detail_handler(caf::blocking_actor *act, caf::actor group = caf::actor()) {
        return [=](detail_atom) -> ContainerDetail { return detail(act, group); };
    }
...
template <typename V> class UuidTree {
  public:
    ...
    insert(const V &value, const utility::Uuid &uuid_before = Uuid(), const bool into = false);
    bool insert(
        const UuidTree<V> &ut,
    ...
    [[nodiscard]] std::optional<UuidTree<V>> intersect(const std::vector<Uuid> &uuids) const;
    ...
    [[nodiscard]] std::optional<Uuid> find_next_at_same_depth(const utility::Uuid &uuid) const;
...
class PlaylistTree : public UuidTree<PlaylistItem> {
  public:
    ...
    bool
    rename(const std::string &name, const utility::Uuid &uuid, UuidTree<PlaylistItem> &child);
    bool rename(const std::string &name, const utility::Uuid &uuid) {
        return rename(name, uuid, *this);
    }

    bool
    reflag(const std::string &flag, const utility::Uuid &uuid, UuidTree<PlaylistItem> &child);
    ...
    [[nodiscard]] std::optional<UuidTree<PlaylistItem> *> find_value(const utility::Uuid &uuid);
    [[nodiscard]] std::optional<const UuidTree<PlaylistItem> *>
    cfind_value(const utility::Uuid &uuid) const;

    [[nodiscard]] std::optional<UuidTree<PlaylistItem> *> find_any(const utility::Uuid &uuid);
    [[nodiscard]] std::optional<const UuidTree<PlaylistItem> *>
...
template <typename V>
utility::UuidVector UuidTree<V>::uuids(const UuidTree<V> &child, const bool recursive) const {
...
std::optional<UuidTree<V>> UuidTree<V>::intersect(const std::vector<Uuid> &uuids) const {
    ...
    if (not children.empty() or std::find(uuids.begin(), uuids.end(), uuid_) != uuids.end()) {
        // preserve..
