...
template <typename K, typename V> class TimeCache {
  ...
  private:
    ...
    void
    add_timepoint_reference(const K &key, const time_point &time, const utility::Uuid &uuid);
...
template <typename K, typename V>
bool TimeCache<K, V>::store_check(
    const K &key, size_t size, const utility::Uuid &uuid, const time_point &out_of_date_time) {
...
bool TimeCache<K, V>::store(
    ...
    if (cache_.count(key)) {
        ...
        if (not shrink_using_out_of_date(
                (max_size_ > _size ? max_size_ - _size : 0), max_count_ - 1, out_of_date_time))
            return false;
...
template <typename K, typename V>
V TimeCache<K, V>::retrieve(const K &key, const time_point &time, const utility::Uuid &uuid) {
    auto it = cache_.find(key);
    if (it == std::end(cache_))
        return V();
    ...
    /*if (noisy){
        ...
        if (retrieve_times.size() == 1024) {
            ...
            avg_sum += sum;
            avg_ct += 1024;
            std::cerr << this << " Cache Size: " << cache_.size() << "  Min retrieve: " <<
    _min << ", Max retrieve: " << _max << ", Average: " << sum/1024 << " (mean), " <<
    retrieve_times[512] << " (median), " << " worst 100: " << last_ten/100 << "   Running
    average " << avg_sum/avg_ct << "\n"; retrieve_times.clear();
        }
    }*/
...
void TimeCache<K, V>::make_entries_hot(
    const std::vector<std::pair<media::MediaKey, utility::time_point>> &keys_and_timepoints) {
...
template <typename K, typename V> void TimeCache<K, V>::unpreserve(const utility::Uuid &uuid) {
