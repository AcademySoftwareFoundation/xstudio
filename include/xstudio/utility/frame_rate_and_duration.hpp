...
namespace utility {
    ...
    class FrameRateDuration {
      ...
      public:
        ...
        FrameRateDuration(const int frames, const double fps)
            : rate_(timebase::to_flicks(1.0 / fps)), duration_(rate_.count() * frames) {}
        FrameRateDuration(const double seconds, FrameRate rat)
            : rate_(std::move(rat)), duration_(timebase::to_flicks(seconds)) {}
        FrameRateDuration(const timebase::flicks flicks, FrameRate rat)
            : rate_(std::move(rat)), duration_(flicks) {}
        FrameRateDuration(const int frames, FrameRate rat)
        ...
        void set_frames(const int frames) { duration_ = rate_ * frames; }
        ...
        bool operator==(const FrameRateDuration &other) const {
            return rate_ == other.rate_ and duration_ == other.duration_;
        }
