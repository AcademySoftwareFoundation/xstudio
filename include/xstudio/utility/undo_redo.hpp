...
template <typename K, typename V> void UndoRedoMap<K, V>::set_max_count(const size_t value) {
    max_count_ = value;
    while (undo_.size() > max_count_) {
...
template <typename K, typename V> void UndoRedoMap<K, V>::push(const K &key, const V &value) {
