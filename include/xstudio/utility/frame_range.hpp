...
class FrameRange {
  public:
    ...
    FrameRange(const FrameRate start, const FrameRate duration, const FrameRate rate)
        : rate_(std::move(rate)), start_(std::move(start)), duration_(std::move(duration)) {}

    FrameRange(const FrameRateDuration &start, const FrameRateDuration &duration)
        : rate_(duration.rate()), start_(start.duration()), duration_(duration.duration()) {}
    ~FrameRange() = default;
    ...
    bool operator==(const FrameRange &other) const {
        return rate_ == other.rate_ and start_ == other.start_ and duration_ == other.duration_;
    }
    bool operator!=(const FrameRange &other) const {
        return rate_ != other.rate_ or start_ != other.start_ or duration_ != other.duration_;
    }
