...
class ThumbnailKey : private std::string {
  ...
  public:
    ...
    ThumbnailKey(const media::AVFrameID &mptr, const size_t hash = 0, const size_t size = 256)
        : std::string(
              fmt::format("{}/{}/{}", to_string(mptr.key()), std::to_string(hash), size)) {}

    using std::string::empty;
    using std::string::substr;

    bool operator==(const ThumbnailKey &o) const {
        return static_cast<const std::string &>(o) == static_cast<const std::string &>(*this);
    }
...
inline fmt::string_view to_string_view(const ThumbnailKey &s) { return {s.data(), s.length()}; }
...
class ThumbnailBuffer {
  public:
    ...
    [[nodiscard]] size_t size() const { return width_ * height_ * channels_ * channel_size_; }
...
struct DiskCacheStat {
    ...
    template <class Inspector> friend bool inspect(Inspector &f, DiskCacheStat &x) {
        return f.object(x).fields(
            f.field("size", x.size_), f.field("count", x.count_), f.field("cache", x.cache_));
    }

    void populate(const std::string &path);
    void
    add_thumbnail(const ThumbnailKey &key, const size_t size, const fs::file_time_type &mtime) {
        add_thumbnail(key.hash(), size, mtime);
    }

    void add_thumbnail(const size_t key, const size_t size, const fs::file_time_type &mtime);
