...
class PlayheadActor : public caf::event_based_actor, public PlayheadBase {
  ...
  private:
    ...
    void
    update_source_multichoice(module::StringChoiceAttribute *attr, const media::MediaType mt);
    void update_stream_multichoice(
        module::StringChoiceAttribute *streams_attr, const media::MediaType mt);

    void restart_readahead_cacheing(const bool all_child_playheads, const bool force = false);
  ...
  protected:
    ...
    [[nodiscard]] bool timeline_mode() const {
        return pinned_source_mode_->value() && timeline_actor_;
    }
    [[nodiscard]] bool contact_sheet_mode() const { return contact_sheet_mode_; }
