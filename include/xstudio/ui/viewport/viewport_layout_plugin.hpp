...
class ViewportLayoutPlugin : public plugin::StandardPlugin {
  public:
    ViewportLayoutPlugin(caf::actor_config &cfg, const utility::JsonStore &init_settings);
  ...
  private:
    ...
    [[nodiscard]] media_reader::ImageSetLayoutDataPtr
...
class ViewportLayoutManager : public caf::event_based_actor {
  public:
    ...
    ~ViewportLayoutManager() override = default;
  ...
  private:
    ...
    std::map<
        std::string,
        std::pair<caf::actor, std::pair<playhead::AutoAlignMode, playhead::AssemblyMode>>>
        viewport_layouts_;
    caf::actor event_group_;
};
