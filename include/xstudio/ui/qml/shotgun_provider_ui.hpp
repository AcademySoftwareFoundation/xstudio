...
class ShotgunThumbnailReader : public ControllableJob<std::pair<QImage, QString>> {
  public:
    ShotgunThumbnailReader(const QString id, const QSize requestedSize)
    ...
    std::pair<QImage, QString> run(JobControl &cjc) override {
        ...
        try {
            ...
            auto shotgun = system_.registry().get<caf::actor>("SHOTGUNDATASOURCE");
    ...
    ShotgunResponse(
        ...
        if (bad_thumbs_.contains(id_) and
            ...
            connect(
                ...
                this,
                &ShotgunResponse::handleFinished);
            connect(
                &watcher_,
                &QFutureWatcher<std::pair<QImage, QString>>::canceled,
                this,
                &ShotgunResponse::handleCanceled);
...
class ShotgunProvider : public QQuickAsyncImageProvider {
  public:
    QQuickImageResponse *
    requestImageResponse(const QString &id, const QSize &requestedSize) override {
        auto response = new ShotgunResponse(id, requestedSize, &pool, bad_thumbs_);
        return response;
    }
