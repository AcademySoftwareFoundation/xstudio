...
inline QString QStringFromStd(const std::string &str) { return QString::fromUtf8(str.c_str()); }
inline std::string StdFromQString(const QString &str) { return str.toUtf8().constData(); }
...
class HELPER_QML_EXPORT TimeCode : public QObject {
    ...
    Q_PROPERTY(unsigned int minutes READ minutes WRITE setMinutes NOTIFY timeCodeChanged)
    Q_PROPERTY(unsigned int seconds READ seconds WRITE setSeconds NOTIFY timeCodeChanged)
    Q_PROPERTY(unsigned int frames READ frames WRITE setFrames NOTIFY timeCodeChanged)
    Q_PROPERTY(double frameRate READ frameRate WRITE setFrameRate NOTIFY timeCodeChanged)
    Q_PROPERTY(bool dropFrame READ dropFrame WRITE setDropFrame NOTIFY timeCodeChanged)
    Q_PROPERTY(
        unsigned int totalFrames READ totalFrames WRITE setTotalFrames NOTIFY timeCodeChanged)
  ...
  public:
    ...
    [[nodiscard]] QString timeCode() const { return QStringFromStd(timecode_.to_string()); }
    ...
    Q_INVOKABLE void setTimeCodeFromString(
        const QString &code, const double frameRate = 30.0, const bool dropFrame = false) {
    ...
    Q_INVOKABLE void setTimeCode(
        ...
        const double frameRate = 30.0,
        const bool dropFrame   = false) {
        timecode_ = utility::Timecode(hour, minute, second, frame, frameRate, dropFrame);
        emit timeCodeChanged();
    }
...
class HELPER_QML_EXPORT PreferencePropertyMap : public ModelPropertyMap {
  ...
  public:
    explicit PreferencePropertyMap(QObject *parent = nullptr) : ModelPropertyMap(parent) {}
    ...
    [[nodiscard]] QVariant defaultValue() const { return values_->value("defaultValueRole"); }
...
class HELPER_QML_EXPORT ModelNestedPropertyMap : public ModelPropertyMap {
  ...
  public:
    explicit ModelNestedPropertyMap(QObject *parent = nullptr) : ModelPropertyMap(parent) {}
...
inline QUrl QUrlFromUri(const caf::uri &uri) {
    if (uri.empty())
        return {};
    return {QStringFromStd(to_string(uri))};
}
...
inline utility::Uuid UuidFromQUuid(const QUuid &quuid) {
    return {StdFromQString(quuid.toString())};
}

inline QVariantMap QVariantMapFromJson(const utility::JsonStore &js) {
    return QJsonDocument::fromJson(QStringFromStd(js.dump()).toUtf8()).object().toVariantMap();
}

inline QVariantList QVariantListFromJson(const utility::JsonStore &js) {
    return QJsonDocument::fromJson(QStringFromStd(js.dump()).toUtf8()).array().toVariantList();
}
...
class HELPER_QML_EXPORT Helpers : public QObject {
  ...
  public:
    ...
    Q_INVOKABLE [[nodiscard]] QString stringFromTimeCode(
        const unsigned int frames, const double frameRate, const bool dropFrame = false) const {
        return QStringFromStd(to_string(utility::Timecode(frames, frameRate, dropFrame)));
    }
    ...
    Q_INVOKABLE [[nodiscard]] QModelIndex qModelIndex() const { return {}; }
    ...
    Q_INVOKABLE [[nodiscard]] QUrl QUrlFromQString(const QString &str) const { return {str}; }
    ...
    Q_INVOKABLE [[nodiscard]] QString validMediaExtensions() const {
        ...
        for (const auto &i : utility::supported_extensions()) {
            if (not result.empty())
                result += " ";
            result += std::string("*.") + i + " " + std::string("*.") + utility::to_lower(i);
        }
    ...
    Q_INVOKABLE [[nodiscard]] QVariant QVariantFromUuidString(const QString &uuid) const {
        return QVariant::fromValue(QUuidFromUuid(utility::Uuid(StdFromQString(uuid))));
    }

    Q_INVOKABLE [[nodiscard]] QUuid nullQUuid() const { return {}; }
    ...
    Q_INVOKABLE [[nodiscard]] QItemSelection createItemSelection() const { return {}; }
    ...
    Q_INVOKABLE [[nodiscard]] QModelIndexList getParentIndexes(const QModelIndexList &l) const;

    Q_INVOKABLE [[nodiscard]] QModelIndexList
    getParentIndexesFromRange(const QItemSelection &l) const;

    Q_INVOKABLE [[nodiscard]] bool
    itemSelectionContains(const QItemSelection &selection, const QModelIndex &item) const {
    ...
    Q_INVOKABLE [[nodiscard]] QColor alphate(const QColor &color, const double alpha) const {
        auto result = color;
    ...
    Q_INVOKABLE [[nodiscard]] QColor saturateLuminate(
        const QColor &color, const double sfactor = 1.0, const double lfactor = 1.0) const {
        float h, s, l, a;
        color.getHslF(&h, &s, &l, &a);
    ...
    Q_INVOKABLE QFuture<bool> inhibitScreenSaverFuture(const bool inhibit = true) const;
    Q_INVOKABLE void inhibitScreenSaver(const bool inhibit = true) const {
        std::ignore = inhibitScreenSaverFuture(inhibit).result();
    }

    Q_INVOKABLE [[nodiscard]] QVariant python_callback(
        QString method_name, QUuid python_plugin_uuid, const QVariant args = QVariant()) const;
...
class HELPER_QML_EXPORT SemVer : public QObject {
  ...
  public:
    ...
    [[nodiscard]] QString version() const { return QStringFromStd(version_.to_string()); }
    void setVersion(const QString &version);

    [[nodiscard]] uint major() const { return version_.major; }
    void setMajor(const uint &major) {
        version_.major = major;
        emit versionChanged();
    }

    [[nodiscard]] uint minor() const { return version_.minor; }
...
class HELPER_QML_EXPORT ClipboardProxy : public QObject {
    ...
    Q_PROPERTY(
        QString selectionText READ selectionText WRITE setSelectionText NOTIFY selectionChanged)
...
class HELPER_QML_EXPORT Plugin : public QObject {
    ...
    Q_PROPERTY(QString qmlWidgetString READ qmlWidgetString NOTIFY qmlWidgetStringChanged)
  ...
  public:
    ...
    [[nodiscard]] QString backend() const { return QStringFromStd(to_string(backend_)); }
...
class HELPER_QML_EXPORT MarkerModel : public JSONTreeModel {
    ...
    Q_PROPERTY(QVariant markerData READ markerData WRITE setMarkerData NOTIFY markerDataChanged)
...
class HELPER_QML_EXPORT PropertyFollower : public QObject {
    ...
    Q_PROPERTY(
        QString propertyName READ propertyName WRITE setPropertyName NOTIFY propertyNameChanged)
