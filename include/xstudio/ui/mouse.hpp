...
struct Signature {
    ...
    enum PointerType {
        ...
        Generic = 0x0001, // A mouse or something acting like a mouse (the core pointer on X11)
        Finger  = 0x0002,
        Pen     = 0x0004,
        Eraser  = 0x0008,
        Cursor  = 0x0010
    ...
    static constexpr void Signature_Modifier_to_str(std::ostream &out, int modifier) {
        ...
        for (int mod_mask = Modifier::ShiftModifier; mod_mask != Modifier::PanActionModifier;
             mod_mask     = mod_mask << 1) {
    ...
    static constexpr const char *Signature_InputType_to_str(Signature::InputType input_type) {
        switch (input_type) {
...
class PointerEvent {
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &out, const PointerEvent &o) {
        out << "PointerEvent " << (int)o.type() << " " << (int)o.buttons() << " "
            ...
            << o.position_in_viewport_coord_sys_.y << " " << o.position_in_viewport_coord_sys_.x
            << " " << o.width_ << " " << o.height_ << " " << o.wheel_delta_.first << " "
            << o.wheel_delta_.second << " " << o.wheel_delta_unit_ << "" << o.pressure_ << " "
            << o.timestamp_ << " " << (int)o.input_type() << " " << (int)o.pointer_type();
        return out;
    }
  ...
  private:
    ...
    float pressure_;
    double timestamp_;
    Imath::V2f position_in_viewport_coord_sys_ =
        Imath::V2f{std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest()};
